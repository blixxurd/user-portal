    ...
    <div class="max-w-md w-full">
      ...
      <form class="mt-8" @submit.prevent="submit" v-if="feedback.length == 0">
        ...
        <div class="input-item mb-4">
          <label for="password" class="pl-1 font-bold text-xs text-gray-700">New Password</label>
          <input v-model="form.password" aria-label="Password" name="password" type="password" autofill="no" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:shadow-outline-teal focus:border-gray-400 focus:z-10 sm:text-sm sm:leading-5" placeholder="enter password">
        </div>
        <div class="input-item mb-2">
          <label for="password" class="pl-1 font-bold text-xs text-gray-700">Verify Password</label>
          <input  v-model="form.passwordVerify" aria-label="Password" name="passwordRepeat" type="password" autofill="no" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:shadow-outline-teal focus:border-gray-400 focus:z-10 sm:text-sm sm:leading-5" placeholder="verify password">
        </div>
